...
4. `docs/specs/2026-08-04-database-and-schema-design.md` — the database and schema spec. **Wins on
   ...
   `docs/specs/erd.html`.
5. `docs/specs/2026-08-04-sdk-wire-contract.md` — the server side of the SDK's 4-endpoint contract,
   plus `POST /auth/player-token` and the build order. **Wins on request/response shapes.** Its
   counterpart is the SDK repo's `docs/specs/sdk-production-implementation.md`, which declares these
   endpoints out of scope. **The contract is frozen: add response fields freely, never remove or
...
**The snapshot is keyed to the session, not the conversation.** The SDK captures it in `Open()` and
delivers it on `POST /sdk/sessions/start` — before any conversation exists, since the web app only
creates one when the player sends a message. A conversation reaches its snapshot via
`conversation.session_id`, and a reopen never rewrites that FK, so "a reopened cycle keeps the original
snapshot" is enforced by the graph rather than by a rule. Delivery is non-blocking and retried, so the
snapshot may land *after* the conversation — an upsert on `session_id` doesn't care about the order.

Three distinct no-data states, all rendered "unavailable" but diagnosed differently: **no row** (never
arrived — Outbox pending, offline, or the SDK failed), **`is_missing = true`** (delivered, but the
game's provider returned nothing), **`degraded_reason` set** (partial — device fields captured, some
provider fields threw). All three are states, never errors.
...
Also: **Saved filter** (private, or shared by a Team Lead), **Agent** (active / on leave).

**Label** (= **tag**; the spec's glossary says "Label / tag") — flat, many per conversation. Two
tables plus a mapping: `label` is the workspace vocabulary, `conversation_label` is what's on a ticket,
and **`subintent_label` is an admin-managed many-to-many meaning "classify as this subintent → apply
these labels."**

That mapping is a table rather than a rule on purpose. p29 shows the same thing as a rule example
("Subintent is a refund request → apply the refund label"), but p29 *also* says rule conditions read
labels — so labels sourced from rules would make a routing rule depend on a tagging rule having fired
first, and evaluation is capped at **exactly one extra pass**. Layering instead: bot picks a subintent →
server applies that subintent's labels and `default_priority` → *then* rules evaluate and may read
those labels. **The bot never invents a tag**, same as never inventing a subintent.

**An applied tag is permanent until an agent removes it.** Reclassification does not swap tags, and
neither does a merge — a tag is a marker someone put on this ticket, not a derived view of its current
subintent. `applied_by IS NULL` marks an auto-applied tag; rule-applied ones are traceable via
`rule_firing`. **Priority is never a tag** — `subintent.default_priority` already exists, and a
`priority_high` label would be a second source of truth. `label.archived_at` retires a tag from new use
while keeping it on old tickets.

Applying or removing a tag is `✓ ✓ ✓` (p43). **Creating a tag and mapping it to subintents is Admin** —
vocabulary and automation are configuration. The spec never says who manages the label vocabulary;
same family of omission as the missing intent rows.

**Session** — the denominator for self-serve rate, and **its id is generated by the SDK**, not the
server. The SDK mints a UUID in `Open()` because it has to go in the webview URL before any network
call succeeds. Accepting it as the PK is what makes `POST /sdk/sessions/start` idempotent via
`ON CONFLICT (id) DO NOTHING` — the Outbox retries and persists offline, so **duplicate delivery is
expected, not exceptional.** No dedupe store, no `Idempotency-Key` table; the header is logged but not
load-bearing. `session.entry_point` is context only — where in the game they tapped support. **Never
classification.**
...
`article_shown`, `article_rejected`, `session_start`, `article_read`, `still_need_help_reached`,
`session_end`, `first_human_reply`, `conversation_resolved`, `conversation_reopened`,
`assignment_returned`, `form_started`, `form_completed`, `form_partial`, `form_skipped`,
`sdk_incident`.

**`article_read` and `still_need_help_reached` are what p40's funnel is made of** — Opened support →
Viewed an article → Reached "still need help?" → Created a ticket. `article_read` also supplies
"articles read per session". It is **not** the same as `article_feedback`: reading an article and
answering "did this help?" are separate signals, and the funnel needs the first while the self-serve
split needs the second.

**Keep `form_partial` distinct from `form_skipped`** — because the form is a conversation, losing the
player halfway through a questionnaire the bot chose to ask is a different failure from the player
declining it up front.

**`sdk_incident` is where `POST /sdk/incidents` lands** — token fetch timed out, webview wouldn't
initialise. An `event` row rather than its own table: low volume, and it inherits workspace scoping,
BRIN and append-only for free. **Something must watch it** — a rising count is how you find out a
release broke support entry for a whole platform, and an unwatched incident stream is exactly the
...
- "Category" and "intent" mean the same thing; "ticket" and "conversation" are interchangeable;
  **"tag" and "label" are the same thing** (the table is `label`); "agent" = CSR.
