    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted">
          Rotating inserts a new secret immediately. The previous one keeps working for a 24-hour grace window so a
          game studio can redeploy without an outage.
        </p>
        <Button size="sm" disabled={rotateMutation.isPending} onClick={() => rotateMutation.mutate()}>
        ...
        <TableBody>
          ...
          {secrets.map((secret) => (
            <TableRow key={secret.created_at}>
              ...
              <TableCell>
                {secret.expires_at ? (
                  <Badge variant="warning">Expires {new Date(secret.expires_at).toLocaleString()}</Badge>
                ) : (
      ...
      <Dialog open={revealed !== null} onOpenChange={(open) => !open && setRevealed(null)}>
        <DialogContent>
          ...
          <div className="flex min-w-0 items-center gap-2 rounded-md border border-zinc-200 bg-surface p-3">
            ...
            <code className="min-w-0 flex-1 overflow-x-auto font-mono text-sm select-all">{revealed}</code>
