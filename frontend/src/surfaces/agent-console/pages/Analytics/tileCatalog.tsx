...
export function renderTile(id: string, data: AnalyticsResponse): ReactNode {
  switch (id) {
    ...
    case 'first-response-time':
      return (
        <NumberTile
          title="First response (avg)"
          value={data.speed.firstResponse.avgSeconds}
          format="duration"
        />
      );
    case 'resolution-time':
      return (
        <NumberTile
          ...
          value={data.speed.resolution.avgSeconds}
          format="duration"
        />
      );
    case 'time-to-claim':
      return (
        <LineChartTile
          title="Time to claim"
          series={data.speed.timeToClaim.series.map((p) => ({
            bucket: p.bucket,
            seconds: p.seconds ?? 0,
          }))}
          dataKeys={['seconds']}
    ...
    case 'bot-containment':
      return (
        <NumberTile title="Bot containment" value={data.bot.containmentRate} format="percent" />
      );
    ...
    case 'article-hit-rate':
      return (
        <NumberTile title="Article hit rate" value={data.bot.articleHitRate} format="percent" />
      );
    ...
    case 'unassigned-queue-depth':
      return (
        <LineChartTile
          ...
          series={data.team.unassignedQueueDepth.series.map((p) => ({
            bucket: p.bucket,
            depth: p.depth,
          }))}
          dataKeys={['depth']}
    ...
    case 'top-cited-articles':
      return (
        <RankedListTile
          ...
          items={data.articles.topCited.map((a) => ({
            id: a.articleId,
            label: a.title,
            count: a.count,
          }))}
    ...
    case 'top-read-articles':
      return (
        <RankedListTile
          ...
          items={data.articles.topRead.map((a) => ({
            id: a.articleId,
            label: a.title,
            count: a.count,
          }))}
