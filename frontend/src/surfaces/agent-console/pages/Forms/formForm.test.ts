...
function field(overrides: Partial<FormField> = {}): FormField {
  return { key: 'k', label: 'Label', type: 'short_text', isRequired: false, position: 0, ...overrides }
}
...
describe('validateFields', () => {
  ...
  it('rejects a duplicate key', () => {
    const errors = validateFields([field({ key: 'a', position: 0 }), field({ key: 'a', position: 1 })])
...
describe('formStatusLabel', () => {
  it('is Archived regardless of publish/draft state', () => {
    expect(formStatusLabel({ archivedAt: '2026-01-01', publishedVersion: 2, hasDraft: true })).toBe('Archived')
  })

  it('is Draft when never published', () => {
    expect(formStatusLabel({ archivedAt: null, publishedVersion: null, hasDraft: true })).toBe('Draft')
  })

  it('is Published v{n} with no pending draft', () => {
    expect(formStatusLabel({ archivedAt: null, publishedVersion: 3, hasDraft: false })).toBe('Published v3')
  })
