  ...
  for (const intent of intents) {
    const rows: SubintentRow[] = intent.subintents
      ...
      .map((s) => ({
        id: s.id,
        name: s.name,
        locked: s.formId !== null && s.formId !== currentFormId,
      }));
    const bulkLocked = rows.some((r) => r.locked);

    if (q === '') {
      if (rows.length > 0)
        groups.push({ id: intent.id, name: intent.name, subintents: rows, bulkLocked });
      continue;
    }
    ...
    if (matching.length > 0)
      groups.push({ id: intent.id, name: intent.name, subintents: matching, bulkLocked });
  }
...
export function ShownForPicker({
  intents,
  selected,
  onChange,
  currentFormId,
          ...
          <div className="flex max-h-80 flex-col gap-2 overflow-y-auto border-t border-slate-200 pt-2">
            {groups.length === 0 ? (
              ...
              groups.map((group) => {
                ...
                const indeterminate =
                  selectedUnlocked.length > 0 && selectedUnlocked.length < unlocked.length;
                ...
                return (
                  <div key={group.id} className="flex flex-col gap-1">
                    ...
                    {group.subintents.map((s) => (
                      <label
                        key={s.id}
                        className={cn('flex items-center gap-2 pl-6', s.locked && 'opacity-50')}
                      >
