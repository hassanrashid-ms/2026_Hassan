...
export function createSubintent(token: string, intentId: string, name: string): Promise<CreateSubintentResponse> {
  return call(`/agent/intents/${intentId}/subintents`, token, { method: 'POST', body: JSON.stringify({ name }) })
}
...
export function moveSubintent(token: string, id: string, intentId: string): Promise<MoveSubintentResponse> {
  return call(`/agent/subintents/${id}/move`, token, { method: 'POST', body: JSON.stringify({ intentId }) })
}

export function mergeSubintent(token: string, id: string, intoId: string): Promise<MergeSubintentResponse> {
  return call(`/agent/subintents/${id}/merge`, token, { method: 'POST', body: JSON.stringify({ intoId }) })
}
...
export function generateKeywords(
  ...
  return call('/agent/articles/generate-keywords', token, { method: 'POST', body: JSON.stringify(input) })
}
