...
1. **No timestamp.** `delivery_state` flips `sent → read` and the _when_ is lost.
2. **No realtime push.** Nothing notifies the _sender_ that their message was
...
Routing — each side receives receipts for _its own_ messages and never an echo
of its own action:
...
_new_ endpoints; this feature adds none.
...
- Grey state uses `currentColor` at reduced opacity so it inherits the bubble's
  ...
  feature is _about_, and own-bubbles are already accent-coloured on both
