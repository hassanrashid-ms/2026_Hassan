...
describe('saveBotConfig', () => {
  ...
  it('leaves an omitted field alone, and resets prompt to the default on an explicit null', async () => {
    ...
    const { rows } = await ownerPool.query(
      `select prompt from bot_config where workspace_id = $1`,
      [workspaceId],
    );
...
describe('saveBotConfig writes its own audit trail', () => {
  ...
  it('rolls the config change back when the audit write fails — no unaudited edit can commit', async () => {
    ...
    const { rows: config } = await ownerPool.query<{
      is_provisioned: boolean;
      prompt: string | null;
    }>(`select is_provisioned, prompt from bot_config where workspace_id = $1`, [workspaceId]);
  ...
  it('stores the prompt before-value in full, so "what did it say before" is answerable', async () => {
    ...
    await withWorkspace(workspaceId, (tx) =>
      saveBotConfig(tx, { workspaceId, actorId, prompt: long }),
    );
    await withWorkspace(workspaceId, (tx) =>
      saveBotConfig(tx, { workspaceId, actorId, prompt: 'short' }),
    );
...
| `change_log` append-only: `UPDATE`/`DELETE` refused             | 2    | `rls.test.ts` — _spec filed this under `changeLog.test.ts`_                              |
| Tenancy: no cross-workspace audit reads                         | 2, 4 | `rls.test.ts` and `changeLog.test.ts` — _spec filed this under `changeLog.test.ts` only_ |
...
index _definitions_ from `pg_indexes`, never an `EXPLAIN` plan. So Task 1 asserts the index exists
