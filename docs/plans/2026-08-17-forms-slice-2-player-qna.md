...
- **The sweeper is part of this slice, not a follow-up.** Without it the gate strands conversations unassigned, which violates _"nothing may prevent a player reaching a human."_
...
2. **`bot_article_rejected` is written in the offer transaction, not deferred.** It records _why_ the handoff happened, which is a fact about the bot turn and true at offer time. Deferring it to terminate would make an abandoned form lose the record of the article rejection entirely.
...
8. **"Does not re-ask a question already answered on the way forward" (§2.7) means does not re-_post_ it.** Navigation stays linear — Next always steps one field forward — and pressing Next over an unchanged prefilled answer fires no request. The test asserts on requests, not on indices.
...
`0002_confirm_phase.sql` is the precedent — `ADD VALUE` inside the migration transaction is fine on PG 17 as long as nothing in the same transaction _uses_ the new value, and nothing here does.
...
export async function seedSubintent(args: {
  ...
  await ownerPool.query(
    ...
    [
      ...
      args.workspaceId,
      args.intentId,
      args.name ?? `Subintent ${id.slice(0, 8)}`,
      args.formId ?? null,
    ],
  );
  return id;
...
export async function seedForm(args: {
  ...
  await ownerPool.query(
    ...
    [
      ...
      args.published === false ? null : new Date(),
    ],
  );
...
async function fixture(options: { withForm: boolean; publishForm?: boolean } = { withForm: true }) {
  ...
  if (options.withForm) {
    const form = await seedForm({
      workspaceId,
      fields: FIELDS,
      published: options.publishForm !== false,
    });
    formId = form.formId;
  }
...
- Produces:
  ...
  export async function answerForm(
    ctx: PlayerContext,
    body: z.infer<typeof FormAnswerBody>,
  ): Promise<AnswerFormResult>;
...
describe('form events', () => {
  ...
  it('snapshots position from the submission version, not the current one', async () => {
    ...
    await answer(f, 'store', 'Google Play');
    const { rows } = await ownerPool.query(
      `select payload from event where type = 'form_field_answered'`,
    );
  ...
  it('stamps session_id when the session is verified', async () => {
    ...
    const { rows } = await ownerPool.query(
      `select session_id from event where type = 'form_field_answered'`,
    );
...
it('returns the resolved form and the answers so far while the card is up', async () => {
  ...
  const res = await request(app)
    .get(`/surface/messages?session_id=${randomUUID()}`)
    .set('Authorization', `Bearer ${f.token}`);
...
Without this job, the gate is strictly worse than no gate: a player who force-quits mid-form leaves a conversation in `bot_active` with no agent assigned and nothing aware of it. That is _"nothing may prevent a player reaching a human"_ violated by accident, so this ships with the slice.
