...
async function main() {
  ...
  try {
    ...
    const targetRes = await pgClient.query(`SELECT id FROM subintent WHERE name = 'Missing Purchase' LIMIT 1`);
    const oldRes = await pgClient.query(`SELECT id FROM subintent WHERE name = 'Item Not Received' LIMIT 1`);
    ...
    if (targetId && oldId) {
      ...
      await pgClient.query(`UPDATE conversation SET subintent_id = $1 WHERE subintent_id = $2`, [targetId, oldId]);
