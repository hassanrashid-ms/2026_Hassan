            ...
            if (call.name === 'classify') {
              ...
              if (classifiedSubintentId === null) {
                ...
                conversationMessages.push({
                  role: 'user',
                  content:
                    '[rejected: already classified this conversation — classify is write-once, do not call it again]',
                });
              }
              continue;
            }
            ...
            if (call.name === ANSWER_FROM_ARTICLE_TOOL_NAME) {
              ...
              if (typeof articleId !== 'string')
                throw new InvalidResponseError(
                  `${ANSWER_FROM_ARTICLE_TOOL_NAME} missing article_id`,
                );
              if (typeof answer !== 'string')
                throw new InvalidResponseError(`${ANSWER_FROM_ARTICLE_TOOL_NAME} missing answer`);

              const cited = searchedArticles.get(articleId);
              if (!cited) {
                conversationMessages.push({
                  role: 'assistant',
                  content: `[${ANSWER_FROM_ARTICLE_TOOL_NAME}(${articleId})]`,
                });
                conversationMessages.push({
                  role: 'user',
              ...
              if (!reply)
                throw new InvalidResponseError(
                  `${ANSWER_FROM_ARTICLE_TOOL_NAME} returned an empty answer`,
                );
              ...
              if (!isGrounded(grounding)) {
                logger.warn(
                  'bot.grounding',
                  'answer not grounded in the cited article — rejected',
                  {
                    workspaceId: input.workspaceId,
                    conversationId: input.conversationId,
                    articleId,
