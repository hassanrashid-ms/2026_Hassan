...
const ERRORS = {
  ...
  conversation_still_open: [409, 'The current conversation is still open. Resolve or close it first.'],
} as const
...
export const newTicketHandler: RequestHandler = async (req, res) => {
  ...
  res.status(201).json({ conversation_id: result.conversationId, status: result.status, message: null })
}
