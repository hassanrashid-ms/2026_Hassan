...
const { agent, declaredField, workspaceMember, intent, subintent, article } =
...
export async function seed(): Promise<void> {
  ...
  try {
    ...
    if (rows[0]) {
      ...
      const { rows: existing } = await owner.query<{ id: string }>(
  ...
  const { adminId, alexId, samId } = await withoutWorkspace(async (tx) => {
    ...
    const [admin] = await tx
      ...
      .onConflictDoUpdate({
        target: agent.email,
        set: { displayName: 'Seed Admin', isAdmin: true },
      })
  ...
  for (const row of insertedArticles) {
    try {
      ...
      logger.warn(
        'weaviate',
        `Failed to index article "${row.title}" into Weaviate: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  ...
  logger.info(
    'db',
    'Workspace secret — printed only here, and only the game backend should hold it:',
  );
