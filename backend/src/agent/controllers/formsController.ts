...
export const updateFormHandler: RequestHandler = async (req, res) => {
  ...
  const result = await updateForm(req.agent!, params.data.id, { name: body.data.name, fields: body.data.fields })
  if (!result.ok) {
...
export const publishFormHandler: RequestHandler = async (req, res) => {
  ...
  if (!result.ok) {
    ...
    const message = result.reason === 'empty_draft' ? 'A published form must have at least one field.' : 'Form has no draft to publish.'
...
export const setFormSubintentsHandler: RequestHandler = async (req, res) => {
  ...
  if (!result.ok) {
    ...
    sendError(res, 422, 'invalid_request', `Unknown or archived subintent id(s): ${result.invalidIds.join(', ')}.`)
    return
  }
  res.status(200).json(result.form)
}
