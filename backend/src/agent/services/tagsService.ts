...
export type RenameTagResult =
  { ok: true; tag: RenameTagResponse } | { ok: false; reason: 'not_found' | 'name_taken' };
...
export type ArchiveTagResult =
  { ok: true; tag: ArchiveTagResponse } | { ok: false; reason: 'not_found' };
...
export async function archiveTag(ctx: AgentContext, id: string): Promise<ArchiveTagResult> {
  return withWorkspace(ctx.workspaceId, async (tx) => {
    ...
    return {
      ok: true,
      tag: { id: row!.id, name: row!.name, archivedAt: row!.archivedAt!.toISOString() },
    };
  });
}
...
export async function attachTag(
  ...
  return withWorkspace(ctx.workspaceId, async (tx) => {
    ...
    const [existing] = await tx
      ...
      .where(
        and(eq(conversationTag.conversationId, conversationId), eq(conversationTag.tagId, tagId)),
      )
      .limit(1);
    ...
    if (existing) {
      await tx
        ...
        .set({ removedAt: null })
        .where(eq(conversationTag.id, existing.id));
    } else {
      await tx
        .insert(conversationTag)
        .values({ workspaceId: ctx.workspaceId, conversationId, tagId });
    }
    return { ok: true };
  });
}
...
export async function getConversationTags(tx: Tx, conversationId: string): Promise<TagView[]> {
  const rows = await tx
    ...
    .where(
      and(eq(conversationTag.conversationId, conversationId), isNull(conversationTag.removedAt)),
    )
    .orderBy(asc(tag.name));
  return rows;
}
