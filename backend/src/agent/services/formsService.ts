...
async function loadVersions(tx: Tx, formId: string) {
  return tx.select().from(formVersion).where(eq(formVersion.formId, formId)).orderBy(desc(formVersion.version))
}
...
export async function listForms(ctx: AgentContext): Promise<FormsListResponse> {
  return withWorkspace(ctx.workspaceId, async (tx) => {
    ...
    const versions = await tx
      ...
      .from(formVersion)
    const subintents = await tx.select({ formId: subintent.formId }).from(subintent).where(isNull(subintent.archivedAt))

    const forms_: FormSummary[] = forms.map((f) => {
      const fVersions = versions.filter((v) => v.formId === f.id)
      const publishedVersions = fVersions.filter((v) => v.publishedAt !== null).map((v) => v.version)
...
export async function createForm(ctx: AgentContext, name: string): Promise<CreateFormResult> {
  return withWorkspace(ctx.workspaceId, async (tx) => {
    ...
    const [versionRow] = await tx
      .insert(formVersion)
      .values({ workspaceId: ctx.workspaceId, formId: formRow!.id, version: 1, fields: [], publishedAt: null })
...
async function loadFormDetail(tx: Tx, formId: string): Promise<FormDetail | null> {
  ...
  const publishedRow = versions.filter((v) => v.publishedAt !== null).sort((a, b) => b.version - a.version)[0]
 ...
 */
export async function updateForm(ctx: AgentContext, formId: string, patch: UpdateFormInput): Promise<UpdateFormResult> {
