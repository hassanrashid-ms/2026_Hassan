...
describe('resolution confirmation — cross-path', () => {
  ...
  it('a tap and the model tool converge on identical rows and events for bot_article', async () => {
    ...
    await withWorkspace(workspaceId, (tx) =>
      applyBotTurn(
        tx,
        { workspaceId, conversationId: conversationB },
        { kind: 'resolve', subintentId: null },
      ),
    );
    ...
    expect({
      status: rowA?.status,
      confirm_phase: rowA?.confirm_phase,
      resolution_source: rowA?.resolution_source,
    }).toEqual({
      status: rowB?.status,
  ...
  it('No on bot_article still produces spec 4 handoff(article_rejected) — regression', async () => {
    ...
    expect({
      status: rowA?.status,
      confirm_phase: rowA?.confirm_phase,
      assigned_agent_id: rowA?.assigned_agent_id,
    }).toEqual({
      status: rowB?.status,
      confirm_phase: rowB?.confirm_phase,
    ...
    expect(messagesA.map((m) => ({ ...m, body: undefined }))).toEqual(
      messagesB.map((m) => ({ ...m, body: undefined })),
    );
