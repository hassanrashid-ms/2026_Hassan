...
describe('forms RLS/tenancy', () => {
  ...
  it('setFormSubintents rejects a subintent id from another workspace via HTTP', async () => {
    ...
    await request(app)
      .post(`/forms/${formIdA}/subintents/set`)
...
describe('forms permissions', () => {
  it('an Agent gets 403 on every route', async () => {
    ...
    await request(app).post(`/forms/${formId}/update`).set('Authorization', `Bearer ${token}`).send({ name: 'Y' }).expect(403)
    ...
    await request(app)
      .post(`/forms/${formId}/subintents/set`)
      .set('Authorization', `Bearer ${token}`)
      .send({ subintentIds: [] })
      .expect(403)
  })
