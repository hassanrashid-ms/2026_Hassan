...
async function conversationRow(id: string) {
  const { rows } = await ownerPool.query(`select status, assigned_agent_id from conversation where id = $1`, [id])
  return rows[0]
}
...
describe('POST /agent/conversations/:id/escalate', () => {
  ...
  it('allows escalating from awaiting_player', async () => {
    ...
    expect((await request(app).post(`/conversations/${conversationId}/escalate`).set('Authorization', `Bearer ${token}`)).status).toBe(200)
  })
  ...
  it('404s on a conversation in another workspace', async () => {
    ...
    const res = await request(app).post(`/conversations/${foreignId}/escalate`).set('Authorization', `Bearer ${token}`)
  ...
  it('422s on a non-uuid id', async () => {
    const workspaceId = await seedWorkspace()
    const { token } = await setupAgent(workspaceId)
    expect((await request(app).post('/conversations/not-a-uuid/escalate').set('Authorization', `Bearer ${token}`)).status).toBe(422)
  })
})
...
describe('POST /agent/conversations/:id/unescalate', () => {
  ...
  it('rejects with 403 when another agent owns it', async () => {
    ...
    await ownerPool.query(`update conversation set status = 'escalated', assigned_agent_id = $2 where id = $1`, [conversationId, otherAgentId])
  ...
  it('404s on a conversation in another workspace', async () => {
    ...
    const otherPlayerId = await seedPlayer(otherWorkspaceId)
    const foreignId = await seedConversation({ workspaceId: otherWorkspaceId, playerId: otherPlayerId })
    await ownerPool.query(`update conversation set status = 'escalated' where id = $1`, [foreignId])
    const res = await request(app).post(`/conversations/${foreignId}/unescalate`).set('Authorization', `Bearer ${token}`)
    expect(res.status).toBe(404)
  })
