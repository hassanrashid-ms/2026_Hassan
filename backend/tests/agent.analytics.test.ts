...
describe('getVolumeMetrics', () => {
  ...
  it('counts opened conversations per bucket from createdAt within range', async () => {
    ...
    expect(result.series).toEqual(
      expect.arrayContaining([expect.objectContaining({ bucket: '2026-08-05', opened: 2 })]),
    );
  });
});
...
describe('getBotMetrics', () => {
  ...
  it('counts closed conversations too, since resolved auto-transitions to closed', async () => {
    ...
    await seedConversation({
      ...
      playerId,
      status: 'closed',
      resolutionSource: 'player_confirmed',
    });
    await seedConversation({
      workspaceId,
      playerId,
  ...
  it('computes self-serve rate as bot/player-driven resolutions over total resolved, excluding agent and admin_forced', async () => {
    ...
    await seedConversation({
      ...
      playerId,
      status: 'closed',
      resolutionSource: 'player_stated',
    });
    await seedConversation({
      workspaceId,
      playerId,
      status: 'closed',
      resolutionSource: 'timed_out',
    });
    await seedConversation({ workspaceId, playerId, status: 'closed', resolutionSource: 'agent' });
    await seedConversation({
      workspaceId,
...
describe('getTopArticles', () => {
  ...
  it('does not throw when other events carry a non-uuid article_id-shaped payload value', async () => {
    ...
    await expect(getTopArticles({ workspaceId }, RANGE)).resolves.toEqual({
      topCited: [],
      topRead: [],
    });
  });
