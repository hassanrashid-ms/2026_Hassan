...
async function seedIntent(workspaceId: string, overrides: Partial<{ name: string; isSystem: boolean; archivedAt: Date | null }> = {}) {
  const { rows } = await ownerPool.query<{ id: string }>(
    `insert into intent (workspace_id, name, is_system, archived_at) values ($1, $2, $3, $4) returning id`,
    [workspaceId, overrides.name ?? 'Billing', overrides.isSystem ?? false, overrides.archivedAt ?? null],
  )
  return rows[0]!.id
}

async function seedArticle(workspaceId: string, intentId: string | null, overrides: Partial<{ title: string; state: string }> = {}) {
  const { rows } = await ownerPool.query<{ id: string }>(
...
describe('GET /intents', () => {
  ...
  it('excludes an archived intent even if it has a published article', async () => {
    const { workspaceId, token } = await fixture()
    const intentId = await seedIntent(workspaceId, { name: 'Old Category', archivedAt: new Date() })
    await seedArticle(workspaceId, intentId, { state: 'published' })
