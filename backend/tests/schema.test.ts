...
describe('schema', () => {
  ...
  it('keys bot_config by workspace_id itself — one row per workspace is structural', async () => {
    ...
    expect(cols.get('prompt')?.nullable).toBe(false);
    ...
    expect(cols.get('rules')?.type).toBe('jsonb');
    expect(cols.get('rules')?.nullable).toBe(false);
  ...
  it('conversation.confirm_phase defaults to none, accepts agent_ask, and rejects an unknown value', async () => {
    ...
    await expect(
      ownerPool.query(`update conversation set confirm_phase = 'bogus' where id = $1`, [
        rows[0]?.id,
      ]),
    ).rejects.toThrow();
  });
  ...
  it('conversation.confirm_phase accepts form', async () => {
    ...
    const { rows } = await ownerPool.query<{ confirm_phase: string }>(
      `insert into conversation (workspace_id, player_id, number, confirm_phase)
       values ($1, $2, 1, 'form') returning confirm_phase`,
      [workspaceId, playerId],
    );
