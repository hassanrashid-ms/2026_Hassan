...
const filterByProperty = vi.fn((name: string) => ({
  equal: (v: unknown) => ({ __filter: v, __property: name }),
}));
...
describe('upsertArticleObject', () => {
  ...
  it('fails within the timeout window when the Weaviate call hangs', async () => {
    const { upsertArticleObject, WEAVIATE_CALL_TIMEOUT_MS } =
      await import('../src/shared/weaviate/articlesIndex.ts');
