...
async function setStatus(conversationId: string, status: string) {
  await ownerPool.query(`update conversation set status = $2 where id = $1`, [conversationId, status])
}
...
describe('POST /surface/new-ticket', () => {
  it('409s while the current conversation is still live', async () => {
    ...
    const { rows } = await ownerPool.query(`select id, status from conversation where player_id = $1`, [playerId])
  ...
  it('closes the resolved conversation and opens a fresh bot_active one', async () => {
    ...
    const { rows: closedEvent } = await ownerPool.query<{ payload: { previous_status: string }; session_id: string }>(
  ...
  it('accepts an unverifiable session_id and stamps the events with null', async () => {
    ...
    expect(res.status).toBe(201)
    const { rows } = await ownerPool.query<{ session_id: string | null }>(`select session_id from event`)
  ...
  it('the next message appends to the new conversation and never reopens the closed one', async () => {
    ...
    const { rows } = await ownerPool.query<{ status: string }>(`select status from conversation where id = $1`, [oldId])
    expect(rows[0]!.status).toBe('closed')
    const { rows: oldMessages } = await ownerPool.query(`select id from message where conversation_id = $1`, [oldId])
