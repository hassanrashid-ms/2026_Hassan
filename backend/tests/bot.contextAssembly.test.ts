...
describe('buildMessages', () => {
  ...
  it('lists the subintent categories in the system prompt while unclassified, and omits them once classified', async () => {
    ...
    const unclassifiedInput = baseInput({
      workspaceId,
      conversationId: unclassifiedConversationId,
    });
    const unclassifiedResult = await withWorkspace(workspaceId, (tx) =>
      buildMessages(tx, unclassifiedInput),
    );
    const unclassifiedSystemMessage = unclassifiedResult.messages[0]!;
    expect(unclassifiedSystemMessage.content).toContain(
    ...
    const classifiedInput = baseInput({
      workspaceId,
      conversationId: classifiedConversationId,
      subintentId,
    });
    const classifiedResult = await withWorkspace(workspaceId, (tx) =>
      buildMessages(tx, classifiedInput),
    );
    const classifiedSystemMessage = classifiedResult.messages[0]!;
    expect(classifiedSystemMessage.content).not.toContain(
      "Classify the player's problem into one of these categories:",
    );
  ...
  it('with 40 messages, keeps the first player message, the last 20, and an elision marker with the dropped count', async () => {
    ...
    const elisionMarker = bodies.filter((b) =>
      new RegExp(`${droppedCount} messages? omitted|elided`).test(b),
    );
  ...
  it('produces the same state block after a simulated five-day gap, plus the gap line', async () => {
    ...
    const recentState = recentResult.messages.find((m) =>
      m.content.includes('conversation state'),
    )!;
    ...
    const stripState = (msgs: typeof recentResult.messages) =>
      msgs.filter((m) => !m.content.includes('conversation state'));
