...
async function offeredAt(startedAt: Date, answers: string[]) {
  ...
  await ownerPool.query(`update conversation set confirm_phase = 'form', status = 'bot_active' where id = $1`, [
    conversationId,
  ])
  ...
  const submissionId = await seedFormSubmission({ workspaceId, conversationId, formId, formVersion: 1, startedAt })
  await ownerPool.query(
    ...
    [
      workspaceId,
      conversationId,
      JSON.stringify({ form_id: formId, form_version: 1, field_count: FIELDS.length, handoff_reason: 'no_article' }),
    ],
  )
