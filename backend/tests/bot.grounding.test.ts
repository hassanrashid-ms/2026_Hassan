...
describe('scoreGrounding', () => {
  ...
  it('accepts a resequenced, shortened rewrite that adds nothing', () => {
    const result = scoreGrounding(
      'Open the shop after you restart — your items are restored automatically.',
      ARTICLE,
    );
    expect(isGrounded(result)).toBe(true);
  });
  ...
  it("accepts the player's own words alongside the article's", () => {
    ...
    const result = scoreGrounding(
      'Your treasure quest items are restored automatically when you restart the game.',
      sources,
    );
    expect(isGrounded(result)).toBe(true);
  });
  ...
  it('rejects an invented timeframe and names it', () => {
    const result = scoreGrounding(
      'Restart the game. Your refund will arrive within 48 hours.',
      ARTICLE,
    );
  ...
  it('scores an answer with no relation to the article near zero', () => {
    const result = scoreGrounding(
      'Equip the crimson banner from your seasonal loadout screen.',
      ARTICLE,
    );
