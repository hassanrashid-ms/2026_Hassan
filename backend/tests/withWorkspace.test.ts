...
describe('withWorkspace', () => {
  ...
  it('rejects a malformed workspace id before opening a transaction', async () => {
    await expect(withWorkspace('not-a-uuid', async (tx) => tx.select().from(player))).rejects.toThrow(
      InvalidWorkspaceId,
    )
    ...
    await expect(withWorkspace('', async (tx) => tx.select().from(player))).rejects.toThrow(InvalidWorkspaceId)
    await expect(
