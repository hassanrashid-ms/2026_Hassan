...
import { EmptyBotPrompt, InvalidRulesPayload, InvalidToolsPayload, InvalidLimitsPayload, resolveBotConfig, saveBotConfig, seedBotConfig as seedBotConfigDomain } from '../src/domain/bot/botConfig.ts'
...
describe('resolveBotConfig', () => {
  ...
  it('cannot tell an absent row from is_provisioned = false — one resolver, one answer', async () => {
    ...
    await seedBotConfig({ workspaceId, isProvisioned: false, prompt: DEFAULT_BOT_PROMPT, rules: buildBaselineRules(), toolsConfig: buildBaselineToolsConfig(), limitsConfig: buildBaselineLimits() })
...
describe('resolveBotConfig / saveBotConfig — limits_config', () => {
  ...
  it('resolves a stored limits_config verbatim into resolvedLimits', async () => {
    const limitsConfig = buildBaselineLimits().map((l) => (l.key === 'max_bot_messages' ? { ...l, value: 10 } : l))
    await seedBotConfig({ workspaceId, isProvisioned: true, prompt: 'p', rules: buildBaselineRules(), toolsConfig: buildBaselineToolsConfig(), limitsConfig })
    const resolved = await withWorkspace(workspaceId, (tx) => resolveBotConfig(tx, workspaceId))
  ...
  it('rejects a limit value outside its bound, naming the bound', async () => {
    const badLimits = buildBaselineLimits().map((l) => (l.key === 'max_bot_messages' ? { ...l, value: 100 } : l))
    await expect(
      withWorkspace(workspaceId, (tx) => saveBotConfig(tx, { workspaceId, actorId, limitsConfig: badLimits })),
  ...
  it('rejects a limits payload missing a catalog key', async () => {
    ...
    await expect(
      withWorkspace(workspaceId, (tx) => saveBotConfig(tx, { workspaceId, actorId, limitsConfig: missingOne })),
    ).rejects.toThrow(InvalidLimitsPayload)
  })
...
describe('seedBotConfig', () => {
  ...
  it('is a no-op when a row already exists', async () => {
    ...
    await withWorkspace(workspaceId, (tx) => saveBotConfig(tx, { workspaceId, actorId, prompt: 'already customised' }))
    const resolved = await withWorkspace(workspaceId, (tx) => seedBotConfigDomain(tx, workspaceId))
