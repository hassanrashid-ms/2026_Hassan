...
describe('POST /agent/conversations/:id/ask-resolved', () => {
  ...
  it('rejects when status is resolved', async () => {
    ...
    const playerId = await seedPlayer(workspaceId)
    const conversationId = await seedConversation({ workspaceId, playerId })
    await ownerPool.query(`update conversation set status = 'resolved', assigned_agent_id = $2 where id = $1`, [conversationId, agentId])
  ...
  it('allows awaiting_player', async () => {
    ...
    const playerId = await seedPlayer(workspaceId)
    const conversationId = await seedConversation({ workspaceId, playerId })
    await ownerPool.query(`update conversation set status = 'awaiting_player', assigned_agent_id = $2 where id = $1`, [conversationId, agentId])
  ...
  it('rejects with 403 when another agent owns it', async () => {
    ...
    await ownerPool.query(`update conversation set status = 'open', assigned_agent_id = $2 where id = $1`, [conversationId, otherAgentId])
