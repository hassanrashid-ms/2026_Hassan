...
describe('reopen', () => {
  it('reopen from resolved posts a handoff line and lands on open, never bot_active', async () => {
    ...
    const [row] = await withWorkspace(workspaceId, (tx) =>
      tx.select({ status: conversation.status }).from(conversation).where(eq(conversation.id, conversationId)),
    )
  ...
  it('awaiting_player -> open posts no system message', async () => {
    ...
    const before = await ownerPool.query(`select id from message where conversation_id = $1`, [conversationId])

    await sendPlayerMessage(ctxFor(workspaceId, playerId), { body: 'reply' })

    const after = await ownerPool.query(`select id from message where conversation_id = $1`, [conversationId])
  ...
  it('a bot-resolved conversation reopens to assignOnHandoff (no previous owner to keep)', async () => {
    ...
    const [row] = await withWorkspace(workspaceId, (tx) =>
      tx.select({ assignedAgentId: conversation.assignedAgentId }).from(conversation).where(eq(conversation.id, conversationId)),
    )
  ...
  it('an agent-resolved conversation with an active previous owner keeps them', async () => {
    ...
    const conversationId = await seedConversation({ workspaceId, playerId })
    await setResolved(conversationId, { resolutionSource: 'agent', assignedAgentId: previousOwnerId })
  ...
  it('an agent-resolved conversation with a deactivated previous owner falls back to assignOnHandoff', async () => {
    ...
    await ownerPool.query(`update agent set status = 'deactivated' where id = $1`, [deactivatedOwnerId])
    ...
    const conversationId = await seedConversation({ workspaceId, playerId })
    await setResolved(conversationId, { resolutionSource: 'agent', assignedAgentId: deactivatedOwnerId })
  ...
  it('conversation_reopened carries the correct previous_resolution_source', async () => {
    ...
    const { rows } = await ownerPool.query<{ payload: { previous_resolution_source: string | null } }>(
