...
import { botFailureNote, HANDOFF_PLAYER_MESSAGES, pickHandoffMessage } from '../src/domain/bot/messages.ts'
...
describe('bot copy', () => {
  ...
  it('says a human is coming and nothing else — no apology, no promised wait, no failure hint', () => {
    for (const line of HANDOFF_PLAYER_MESSAGES) {
      expect(line.toLowerCase()).not.toMatch(/sorry|apolog|wrong|error|failed|unable|minute|hour|shortly|soon/)
    }
  })
