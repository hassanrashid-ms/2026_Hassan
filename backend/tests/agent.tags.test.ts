...
describe('POST /tags', () => {
  it('creates a fresh tag with 201', async () => {
    ...
    const res = await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'VIP' }).expect(201)

    expect(res.body).toEqual({ id: expect.any(String), name: 'VIP', colorIndex: expect.any(Number) })
  })
  ...
  it('returns the existing active tag as-is for an exact duplicate (200, no-op)', async () => {
    ...
    const first = await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'VIP' }).expect(201)
    const second = await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'VIP' }).expect(200)
  ...
  it('un-archives a matching archived tag instead of creating a duplicate', async () => {
    ...
    const created = await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'VIP' }).expect(201)
    await request(app).post(`/tags/${created.body.id}/archive`).set('Authorization', `Bearer ${token}`).expect(200)

    const revived = await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'VIP' }).expect(200)

    expect(revived.body.id).toBe(created.body.id)

    const { rows } = await ownerPool.query<{ archived_at: string | null }>(`select archived_at from tag where id = $1`, [
      created.body.id,
    ])
...
describe('GET /tags', () => {
  it('lists only active tags, alphabetically', async () => {
    ...
    await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'Zebra' }).expect(201)
    const archived = await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'Archived' }).expect(201)
    await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'Alpha' }).expect(201)
    await request(app).post(`/tags/${archived.body.id}/archive`).set('Authorization', `Bearer ${token}`).expect(200)
  ...
  it('filters by normalizedName prefix', async () => {
    ...
    await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'VIP Whale' }).expect(201)
    await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'Refund' }).expect(201)

    const res = await request(app).get('/tags?query=vip').set('Authorization', `Bearer ${token}`).expect(200)
...
describe('PATCH /tags/:id', () => {
  it('renames a tag', async () => {
    ...
    expect(res.body).toEqual({ id: created.body.id, name: 'Whale', colorIndex: created.body.colorIndex })
  })

  it('409s on a name collision with another active tag', async () => {
    const workspaceId = await seedWorkspace()
    const { token } = await seedAgentSession(workspaceId)
    await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'Whale' }).expect(201)
    const created = await request(app).post('/tags').set('Authorization', `Bearer ${token}`).send({ name: 'VIP' }).expect(201)
