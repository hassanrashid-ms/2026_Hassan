...
import { closeSocketServer, createSocketServer, getIo } from '../src/shared/realtime/socketServer.ts'
...
function captureEmits() {
  ...
  vi.spyOn(getIo(), 'to').mockImplementation(
    (room) => ({ emit: (event: string) => { emits.push({ room: String(room), event }); return true } }) as never,
  )
  return emits
}
...
describe('POST /surface/resolution-answer', () => {
  ...
  it('posts the decline as a player message and emits it to both rooms on no', async () => {
    ...
    const res = await request(app).post('/resolution-answer').set('Authorization', `Bearer ${token}`).send({ helped: false })
    expect(res.status).toBe(200)
