...
> — not the _Build order_ section below, which uses different numbering) are done:
...
the forms all live in _this_ repo's web surface — not duplicated in C#, JS, Kotlin and Swift.
...
(a different numbering than the _Build order_ section below): `POST /auth/player-token`, the four
...
console, reporting. Item 3 ("The core loop end to end") of the _Build order_ section below.
...
3. **The core loop end to end.** `POST /conversations` and `POST /messages` with correct sequencing and
   ...
   touched.
4. **Bot and Knowledge Base.** Build the _handoff_ path first and the clever part second: a bot that
...
- **First reply means first _human_ reply.** Bot, system and internal notes don't count.
